...
Observerパターンは、Publish/SubscriberまたはPublish/Listenerとしても知られている。Stateパターンで、最初のイベント駆動型アーキテクチャを定義したが、Observerパターンで、本当に新しい抽象化レベルに達することができる。

### 説明

Observerパターンの背後にある考え方は単純で、あるイベントをサブスクライブすると、サブスクライブした多くのタイプにある動作がトリガーされる。なぜこれがそんなに面白いのか？なぜなら、イベントをその可能なハンドラから切り離すからである。

### 目的

Observerパターンは、1つのイベントをトリガーとして多くのアクションを実現する場合に特に有効。また、イベント後に実行されるアクションの数が事前に分からない場合や、近い将来にアクションの数が増える可能性がある場合にも特に有効。再開するには、以下のようにする。

- 1つのイベントが1つ以上のアクションを引き起こすことができるイベント駆動型アーキテクチャを用意する
- 実行されるアクションとそのトリガーとなるイベントの結合を解除する 同じアクションをトリガーするイベントを複数用意する

### Notifier

Observerパターンの根幹を理解するために、最も単純なアプリケーションを開発する。Publisher 構造体は、イベントをトリガーする構造体なので、新しいオブザーバーを受け入れ、必要に応じて削除する必要がある。Publisher構造体がトリガーされると、関連するデータとともに新しいイベントをすべてのオブザーバーに通知する必要がある
